...
class SolaceACLPublishExceptionTask(su.SolaceTask):
    ...
    def get_func(self, solace_config, vpn, acl_profile_name, topic_syntax):
        ...
        return su.get_configuration(solace_config, path_array, 'publishTopicException')
    ...
    def create_func(self, solace_config, vpn, acl_profile_name, topic_syntax, publish_topic_exception, settings=None):
        ...
        path = '/'.join([su.SEMP_V2_CONFIG, su.MSG_VPNS, vpn, su.ACL_PROFILES, acl_profile_name, su.ACL_PROFILES_PUBLISH_TOPIC_EXCEPTIONS])
...
def run_module():
    ...
    solace_acl_publish_task = SolaceACLPublishExceptionTask(module)
